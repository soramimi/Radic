...
- FreeRDPの`cliprdr`静的チャネルを使い、`CF_UNICODETEXT`によるUnicodeプレーンテキストを双方向共有する。画像、HTML、ファイルなどの形式は未対応。
...
- `cliprdrMonitorReady`でクライアント能力を通知し、ローカルの`QClipboard::dataChanged`時は`ClientFormatList`を送信する。リモートからは`ServerFormatList`→`ClientFormatDataRequest`→`ServerFormatDataResponse`の順でテキストを取得する。
...
- クリップボード共有はプレーンテキストのみ対応（画像・ファイルは未対応）
...
10. **テキストのクリップボード共有を追加** — `cliprdr`による双方向`CF_UNICODETEXT`共有を実装。能力交渉・方向別FeatureMask・Qt側の再通知ループ防止に加え、Windows側のコピー直後に`CB_RESPONSE_FAIL`が返る問題を遅延要求と世代管理付きリトライで回避
